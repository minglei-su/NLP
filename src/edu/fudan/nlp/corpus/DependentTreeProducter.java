...
public class DependentTreeProducter {
	...
	public  void write(InstanceSet ins,String writePath,String rulePath) throws IOException{
		...
		//			if(te.getLabel().getCore().getId()!=-1)
						wr.write(te.getLabel().getId()+"\t"+te.getLabel().getData()+"\t"+te.getLabel().getTag()+"\t"+te.getLabel().getCore().getId()+"\t"+this.getDepClass(te.getLabel())+"\n");
		//			else
	...
	public static void main(String[] args) throws IOException{
		...
		InstanceSet ins = MyTreebankReader.readNewTrees("./data3", "txt",Charset.forName("GBK"));
		rp.write(ins, "./test-data/result.txt", "./test-data/headrules.txt");
	}
