...
public class YamadaParser extends Inferencer {
	 ...
	 */
	protected String defaultPOS = null;
	...
	private Results<DependencyTree> _getBestParse(Sentence sent){
		float score = 0;
	...
	private float[][] estimateActions(ParsingState state) throws UnsupportedDataTypeException {
		...
		for (int i = 0; i < guess.length; i++) {
			...
			if (action.matches("L"))
				...
				result[0][i] = 2;
			result[1][i] = (float) Math.exp(ret.predScores[i]);
			total += result[1][i];
		}
		for (int i = 0; i < guess.length; i++) {
	...
	public DependencyTree getBestParse(Sentence sent)	{
		return _getBestParse(sent).getPredAt(0);
	}
