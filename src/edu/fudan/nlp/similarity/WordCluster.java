...
public class WordCluster implements Serializable{
	...
	public void read(Reader reader) {
		while (reader.hasNext()) {
			...
			int prechar = alpahbet.lookupIndex("begin");
			wordProb.adjustOrPutValue(prechar, 1, 1);
			for (int i = 0; i < content.length()+1; i++) {
				int j;
				if(i<content.length()){
					String c = String.valueOf(content.charAt(i));
					j = alpahbet.lookupIndex(c);					
				}
				else{
					j = alpahbet.lookupIndex("end");					
				}
				wordProb.adjustOrPutValue(j, 1, 1);
				...
				map.adjustOrPutValue(j, 1, 1);

				TIntHashSet left = leftnodes.get(j);
				if(left==null){
					left = new TIntHashSet();
					leftnodes.put(j, left);
...
//		int[] idx = MyCollection.sort(wordProb);
		...
		int[] idx = MyHashSparseArrays.trim(wordProb, 0.99f);
	 ...
	 * @throws Exception
	 */
	protected void saveTxt(String file) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		String file = cl.getOptionValue("path", "./tmpdata/SogouCA.mini.txt");
