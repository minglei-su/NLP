...
public class LinearViterbi extends AbstractViterbi {
	 ...
	 */
	public LinearViterbi(LinearViterbi viterbi) {
	...
	@Override
	public Results<int[]> getBest(Instance carrier) {

		Node[][] node = initialLattice(carrier);

		doForwardViterbi(node, carrier);

		Results<int[]> res = getPath(node);
	 ...
	 * @return 最优路径及其得分
	 */
	protected Results<int[]> getPath(Node[][] lattice) {
